...
- **Offline review (ruled OUT, source looks clean):** `hls_read`, `ensure_segment`, `prefetch_take`, `prefetch_main` all reviewed — correct locking, bounds checks (`seg < g_segCount`), null guards (`g_segs[seg]`), slot lifecycle. So the crash is NOT in segment fetch/serve. Remaining suspects to check WITH /crashlog: `avformat_open_input`/demux-probe over the concatenated-TS AVIO callback, `decode_thread_main` SW-decode handling of VOD, or a delayed effect from the preceding 8-cycle rapid cast/stop stress (possible fd/thread accumulation — though seg_readahead_stop teardown looked correct and the stress cycles all responded cleanly). Read /crashlog FIRST; don't pre-emptively "fix" without the fault address.
